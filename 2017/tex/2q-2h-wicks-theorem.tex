...
\thmproof{
  ...
  +
    ...
    \no{\ctr{Q(}{q}{_k)}{q} Q(q_k)q}
  \end{align*}
  since $\no{\ctr{Q(}{q}{_k)}{q} Q(q_k)q}=(-)^{n-k}\no{q_1\cd\ctr{}{q}{_k}{q}q_kq\cd q_n}$ and $\ctr{}{q}{_k}{q} q_kq=[q_k,q]_+$ when $q$ is a particle creation operator (see \cref{dfn:contraction}).
}
\end{samepage}
\end{lem}
