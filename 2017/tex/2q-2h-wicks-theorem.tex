...
\thmproof{
  ...
  If $q$ is a quasiparticle annihilation operator then $\no{Qq}=Qq$ and all cross-contractions vanish, so the statement holds trivially.
  If $q$ is a quasiparticle creation operator then $\no{Qq}=(-)^nqQ$ and, using an anticommutator relation derived in the appendix (\cref{prop:pull-through-relation}),
  \begin{align*}
    Qq
  =
    (-)^n qQ
  ...
  +
    ...
    \no{\ctr{Q(}{q}{_k)}{q} Q(q_k)q}
  \end{align*}
  since $\no{\ctr{Q(}{q}{_k)}{q} Q(q_k)q}=(-)^{n-k}\no{q_1\cd\ctr{}{q}{_k}{q}q_kq\cd q_n}$ and $\ctr{}{q}{_k}{q} q_kq=[q_k,q]_+$ when $q$ is a quasiparticle creation operator (see \cref{dfn:contraction}).
}
\end{samepage}
\end{lem}
